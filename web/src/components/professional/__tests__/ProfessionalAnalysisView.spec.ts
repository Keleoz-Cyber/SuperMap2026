...
describe('单候选联动与参数快照', () => {
  ...
  it('基线评估：enhanced_evidence_available=false 时显示基线指标且不请求增强证据', async () => {
    ...
    expect(wrapper.find('[data-test="baseline-only-note"]').text()).toContain('目前只显示总体指标')
    expect(wrapper.get('[data-test="evaluation-conclusion"]').text()).toContain('先看总体误差')
    expect(wrapper.get('[data-test="evaluation-conclusion"]').text()).toContain('还没有逐折结果和残差图')
    ...
    expect(wrapper.get('[data-test="baseline-metrics"]').text()).toContain('RMSE 表示预测值通常会偏离观测值多少')
  ...
  it('增强证据可用时结论先说明逐折、残差与泄漏检查状态', async () => {
    ...
    expect(conclusion.text()).toContain('评估结果完整')
    expect(conclusion.text()).toContain('训练和验证分组没有重叠')
