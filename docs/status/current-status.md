...
> 更新时间：2026-08-11（v0.9.0 视觉产品、成果级分析、三维联动与 AI 辅助集成完成，发布候选待合并）。本文是开发人员和开发 Agent 判断“现在做到哪一步”的唯一状态入口。数据细节见 [电阻率](../data/resistivity.md)、[微震](../data/microseismic.md)、[瓦斯](../data/gas.md) 和 [数据契约](../data/contracts.md)；目标产品见 [产品蓝图](../product-blueprint.md)。
...
- **v0.9.0 答辩级视觉产品与全流程体验重构（`feat/v0.9.0-visual-product` 分支，发布候选）**：全站按「深地极光」设计系统重构，真实浏览器截图为验收门。设计与验收口径：[设计规格](../superpowers/specs/2026-08-10-v0.9.0-visual-product-redesign-design.md)、[实施计划](../superpowers/plans/2026-08-10-v0.9.0-visual-product-redesign.md)。
  - 全局壳与视觉系统：`AppShell/AppHeader`（品牌、案例上下文、服务状态、全局「导入数据 / 新建建模」、回收站；答辩模式已随 V6 成果工作台重构退役）+ S1 token（墨绿黑画布、青结构色、核心金、四案例辅助色）+ 动效 token（含 `prefers-reduced-motion`）+ `AsyncState` 六类统一状态 + `CaseStageNav` 四阶段导航；版本徽标移至全局头（`test_version_consistency` 同步更新）。
  ...
  - 成果级分析与 AI：正式成果只读摘要以 `result_id + grid_sha256` 绑定完整网格统计、p25/p75 组成、深度层段、连通区、模型证据与确定性发现；右栏规则研判始终可用，DeepSeek 为服务端可选增强并带 evidence hash/prompt 版本/证据引用，未配置时类型化 `unavailable`。组件、四相机、Volume/Slice/Contour 与权威切片经 SuperMap3D v2 协议联动；桌面 1920×1080 页面级横纵溢出均 ≤1 px。
  - V6 成果工作台：成果页重构为一屏工作台——成果专用顶栏（品牌/五段导航/案例选择/导出分析报告，全局头在此页隐藏）、成果摘要条（算法/网格/坐标/变量 + 有效样本/R²/公共有效集/正式成果状态，无指纹长串与完整哈希）、三栏主舞台（显示工具 328px / 真实体场景 / 分析研判 390px）与四标签证据窗（综合分析/切片与异常/模型证据/数据溯源，七类证据归组不丢失，资产身份与导出迁入数据溯源）；光照与渐变透明度在后端 profile 与前端降级路径均默认关闭、包围盒默认开启；XYZ 坐标架原点外移出包围盒、轴长为对应跨度 1.25×（合同 1.2–1.3）、深度刻度独立成尺；三维只标注后端排序前三的主导组件，完整组件仍保留在研判区；答辩模式入口/路由/组件/store/专用测试完整删除，旧 /presentation 深链经 catch-all 回首页；相机只调整确定性视角与距离，不改数据几何。
  ...
  - 集成回归（2026-08-11 fresh 运行）：后端非 `local_data` `1895 passed, 32 deselected`；前端 vitest `501 passed`、type-check/build 通过；Mock E2E `41 passed`；成果级真实 SDK live `1 passed`。证据见 `docs/evidence/v0.9.0-result-analysis-live/`；既有 IDW RuntimeWarning 与 jsdom/ECharts 测试警告仍存在，但无失败。
  - V6 验收回归（2026-08-11 fresh 运行）：后端非 `local_data` `1895 passed, 32 deselected`；前端 vitest `498 passed`、type-check/build 通过；Mock E2E 既有门未改；真实 SDK live `2 passed`（1920×1080 + 1440×900：一屏无滚动/溢出、三栏 328/1494/390、默认光照/渐变关闭、关闭辅助层后体场本体高 0.7411/宽 0.2716、三维 3 个主导标注、坐标架原点在包围盒外 1.25×、四证据标签与规则/AI 双页可切、零未解释控制台/页面/网络失败）。证据见 `docs/evidence/v0.9.0-v6-result-workbench/`。
  - 当前边界：真实 GPU 已验证组件列表→三维聚焦，三维标注真实鼠标 pick 命中率尚无独立 GPU 门；DeepSeek 真实外部成功调用未在本机配置密钥，本轮只验证结构化假服务单测与未配置降级；局部坐标/显示锚点、非真实地理配准及 `manual_required` 不变。
