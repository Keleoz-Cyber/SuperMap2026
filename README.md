...
当前版本：**0.9.1**。发布包与历史版本见 [GitHub Releases](https://github.com/Keleoz-Cyber/SuperMap2026/releases)。

## 直接体验

评测组使用 Windows x64 免安装包：下载 `GeoModelingPlatform-0.9.1-win-x64.zip`，完整解压后双击 `启动平台.cmd`。包内已包含 Python 运行时、前端、SuperMap3D SDK、SQLite 和三个内置案例，无需安装 Python、Node.js、Docker 或 iServer。
...
- 专业分析：半变异函数、人工确认各向异性、旋转邻域、误差尺度、异常连通区。
...
- 随机森林离散度不是概率置信区间；AI 只解释确定性证据，不替代模型和人工判断。
