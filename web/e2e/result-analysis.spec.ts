...
test.describe('成果级分析工作台（mock 协议帧）', () => {
  ...
  test('分析接入、组件/相机/切片/AI 联动与证据带', async ({ page }) => {
    ...
    // 证据带：深度趋势/组件比较/当前切片/输入样本/溯源
    await page.getByTestId('ge-tab-depth').click()
    await expect(page.getByTestId('ge-pane-depth')).toContainText('45.1%')
    await page.getByTestId('ge-tab-components').click()
    await expect(page.getByTestId('ge-pane-components')).toContainText('网格支持体积估计')
    await page.getByTestId('ge-tab-slice').click()
    await expect(page.getByTestId('ge-slice-heatmap')).toBeVisible()
    await page.getByTestId('ge-tab-input').click()
    await expect(page.getByTestId('ge-pane-input')).toContainText('输入样本')
