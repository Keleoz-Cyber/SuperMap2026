...
> 更新时间：2026-08-13。本文记录当前仓库采用的瓦斯数据合同、坐标/单位口径、官方基线和分析边界。当前实现状态以[当前状态](../status/current-status.md)为准。
...
- 机器学习适用性门：58 个样品和 28 个独立 XY 位置不足以支持当前 ML 合同，随机森林与克里金残差随机森林均被明确禁用，不生成伪候选。
