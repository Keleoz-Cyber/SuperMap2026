...
| `停止平台.cmd` | 停止平台进程。 |
| `启动平台.command` | macOS ARM64 最终用户双击启动；构建时写入可执行权限。 |
| `停止平台.command` | macOS ARM64 停止平台进程。 |
| `THIRD_PARTY_NOTICES.txt` | 第三方组件声明（Python 运行时、SuperMap3D SDK、前端依赖等）。 |

## 说明

- 发行包内的 `使用说明.txt`、`portable-manifest.json`（逐文件 SHA-256 清单）与 `GeoModelingPlatform.exe`/`GeoModelingPlatform`（支持 `start`/`doctor`）在构建时生成，不在本目录维护。
