...
- **v0.9.0 答辩级视觉产品与全流程体验重构（`feat/v0.9.0-visual-product` 分支，发布候选）**：全站按「深地极光」设计系统重构，真实浏览器截图为验收门。设计与验收口径：[设计规格](../superpowers/specs/2026-08-10-v0.9.0-visual-product-redesign-design.md)、[实施计划](../superpowers/plans/2026-08-10-v0.9.0-visual-product-redesign.md)。
  ...
  - V6 成果工作台：成果页重构为一屏工作台——成果专用顶栏（品牌/五段导航/案例选择/导出分析报告，全局头在此页隐藏）、成果摘要条（算法/网格/坐标/变量 + 有效样本/R²/公共有效集/正式成果状态，无指纹长串与完整哈希）、三栏主舞台（显示工具 328px / 真实体场景 / 分析研判 390px）与四标签证据窗（综合分析/切片与异常/模型证据/数据溯源，七类证据归组不丢失，资产身份与导出迁入数据溯源）；光照与渐变透明度在后端 profile 与前端降级路径均默认关闭、包围盒默认开启；XYZ 坐标架原点外移出包围盒、轴长为对应跨度 1.25×（合同 1.2–1.3）、深度刻度独立成尺；答辩模式入口/路由/组件/store/专用测试完整删除，旧 /presentation 深链经 catch-all 回首页；相机取景只调距离不改数据几何。
  ...
  - V6 验收回归（2026-08-11 fresh 运行，最终 HEAD）：后端非 `local_data` `1895 passed`；前端 vitest `497 passed`（答辩模式专用测试随功能删除）；Mock E2E `39 passed`；真实 SDK live `2 passed`（1920×1080 + 1440×900：一屏无滚动/溢出、三栏 328/1494/390、默认光照/渐变关闭、体场高度占比 0.6958、坐标架原点在包围盒外 1.25×、四证据标签与规则/AI 双页可切、零控制台/页面/未解释网络失败）。证据见 `docs/evidence/v0.9.0-v6-result-workbench/`。
