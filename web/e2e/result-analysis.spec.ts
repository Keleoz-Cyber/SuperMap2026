...
test.describe('成果级分析工作台（mock 协议帧）', () => {
  ...
  test('分析接入、组件/相机/切片/AI 联动与证据带', async ({ page }) => {
    ...
    await expect(lowCard).toContainText('可能与含水')
    await expect(lowCard).toContainText('提示地下介质电性结构存在差异')
    await expect(lowCard).toContainText('建议：')
    await expect(lowCard).toContainText('不能只凭低阻结果判断这里有水')
    // 技术证据默认收起，仍可追溯原有发现与 A/B 组件
    await expect(page.getByTestId('result-interpretation')).toContainText('A 区覆盖约 1200，是当前最大的连续区域')
    await expect(page.getByTestId('component-1')).toContainText('模型覆盖范围估计（三维网格）')
    ...
    await expect(page.getByTestId('ge-pane-slices')).toContainText('模型覆盖范围')
    ...
    await expect(page.getByTestId('ai-perspective-spatial_pattern')).toContainText('高值主要集中在 -650～-500m')
