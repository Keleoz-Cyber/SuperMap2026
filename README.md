...
- **v0.9.0 机器学习空间预测扩展（本分支，发布候选）**：机器学习直接进入既有「调参实验室 → 候选比较 → 成果融合工作台」流程，不另建割裂的 AI 中心。新增 `random_forest_spatial`（坐标派生特征的确定性随机森林空间回归）与 `kriging_rf_residual`（普通克里金基线 + 折外残差随机森林校正）；两者均使用空间交叉验证，残差模型只以内部空间折的折外克里金残差训练，避免把训练点拟合残差泄漏给验证。成果可在同一 SuperMap3D 主场景切换预测结果、模型离散度、克里金基线和残差校正场；离散度仅作树模型分歧参考，不代表概率意义上的不确定范围，残差校正使用零中心冷暖色带。数据适用性门按有效样本与独立 XY 空间组判断：电阻率支持两种 ML，微震只有 22 个 XY 组，仅开放经确认的实验性随机森林，58 点瓦斯明确不建议并禁用 ML。模型可与相同折分指纹和公共有效集的普通克里金同口径比较；ML 未提升时界面明确显示「未优于普通克里金」，不强行推荐。真实数据与 SuperMap SDK 多字段/缓存门见 [docs/evidence/v0.9.0-ml-spatial-prediction/](docs/evidence/v0.9.0-ml-spatial-prediction/)。
...
- **v0.9.0 答辩级视觉产品与全流程体验重构（本分支，发布候选）**：全站重构为「深地极光」综合指挥舱形态，视觉完成以真实数据 + 真实浏览器截图为验收门。设计与验收口径见 [设计规格](docs/superpowers/specs/2026-08-10-v0.9.0-visual-product-redesign-design.md) 与 [实施计划](docs/superpowers/plans/2026-08-10-v0.9.0-visual-product-redesign.md)。
  - **全局产品壳与 S1 设计系统**：`AppShell/AppHeader` 持久壳（品牌、案例上下文、服务状态、全局「导入数据 / 新建建模」、回收站），S1「深地极光」token（墨绿黑画布、低饱和青结构色、少量核心金、四案例辅助色：电阻率金 / 微震紫 / 瓦斯翡翠 / 自定义青），统一动效 token 并支持 `prefers-reduced-motion`；`AsyncState` 统一加载/空/错误/离线/降级/NoData 六类状态（每个状态回答「发生了什么/受影响能力/下一步」）。
  ...
  - **成果级规则研判与可选 AI 辅助**：后端按 `result_id + grid_sha256` 输出完整网格统计、p25/p75 组成、3D 层段、高值连通区、模型证据与确定性发现；组件列表、四相机、Volume/Slice/Contour 和权威切片与 SuperMap3D v2 同身份联动。DeepSeek 密钥仅从服务端环境变量读取，未配置/超时/错误均类型化降级且不影响规则研判；AI 结论保留 evidence hash、prompt 版本与证据引用，不输出隐藏推理或虚构置信度。
  ...
  - 证据：真实 SDK/GPU 验收（三案例指挥舱切换渲染、图表→三维联动、手机视口；答辩模式已随 V6 退役，其历史彩排证据保留于该目录）见 [docs/evidence/v0.9.0/](docs/evidence/v0.9.0/)；成果级规则分析、组件聚焦/相机/切片、AI 未配置降级与 1920×1080 一屏门见 [docs/evidence/v0.9.0-result-analysis-live/](docs/evidence/v0.9.0-result-analysis-live/)。
