...
你接手的是 SuperMap2026 / GeoModelingPlatform 的 v0.9.0 总任务。该任务严格串行：后端 Agent 完成并提交汇报后，前端 Agent 才开始；前端完成并提交汇报后，集成负责人再开始。每一棒必须同时收到“本棒提示词 + 上一位 Agent 的完整汇报”，并首先核验实际 Git 状态。详细分工见同目录提示词。不要再做静态视觉稿，也不要只补文档或堆测试；目标是把现有成果页变成由真实成果网格驱动、能在浏览器演示的分析工作台。
