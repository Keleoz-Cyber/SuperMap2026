...
describe('AppShell', () => {
  ...
  it('renders explicit stable global navigation without context-disabled entries', () => {
    ...
    expect(wrapper.find('[data-test="shell-brand"]').exists()).toBe(false)
    expect(wrapper.get('[data-test="shell-platform-title"]').text()).toBe('地质属性三维建模与智能分析平台')
    expect(wrapper.get('[data-test="shell-home-link"]').text()).toBe('首页')
    expect(wrapper.find('[data-test="shell-nav-cases"]').exists()).toBe(false)
    expect(wrapper.find('[data-test="shell-nav-ingest"]').exists()).toBe(false)
