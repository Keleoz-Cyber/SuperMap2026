...
test.describe('v0.8.0 第三批：瓦斯预置官方成果真实 SDK live 门', () => {
  ...
  test('用例 1：首页 → 工作台 → 成果页身份链 → 五模式渲染门与交互 → 普通刷新', async ({
    ...
    await expect(gasCard).toContainText('瓦斯含量')
    ...
    await expect(page.getByTestId('workspace-data')).toContainText(String(GAS_ROW_COUNT))
    await expect(page.getByTestId('workspace-data')).toContainText('瓦斯含量')
    await expect(page.getByTestId('workspace-data')).toContainText(EXPECTED_VALUE_UNIT)
