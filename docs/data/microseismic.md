...
> 数据契约通用规则见 [contracts.md](contracts.md)。本文同时区分：v0.2a代码已经验证的审计事实，以及2026-07-20已经确认但尚未写入代码/config的局部三维建模规则。

## 1. 当前结论

微震 v0.2a 已合并到 `main`（PR #2），完成数据审计与标准化底座：22 个 DAT 全部可读取、可追溯，2,006 条源记录进入标准表，三张标准表、一维累计距离、契约验证、问题清单和审计报告均可复现。

目前已确定一套**独立案例使用的局部三维坐标**、`WL/2`到深度的换算、Vx单位和无效值规则，因此微震局部三维插值不再因这些业务决策而阻断。但当前代码和 `config/microseismic.yaml` 仍是v0.2a实现，尚未生成X/Y/Z或正式插值结果；绝对地理配准也仍然未知，不能与电阻率或瓦斯空间叠加。
...
- 除这一条无效值外，其余2,005条有限数值全部进入候选建模集合；
- 不采用论文所述“删除80条异常值”、3.59%比例、线性填补或5点IDW填补规则；若将来进行稳健性实验，必须作为独立候选处理，不能修改原始观测；
- W28以及没有正式DAT的点不进入建模。
...
- **清洗比例冲突**（`CLEANING_RATE_CONFLICT`）：论文称删除80条异常值、占3.59%，但80/2,005≈3.99%；仅保留为论文冲突证据，不采用。
- **清洗方法冲突**（`CLEANING_METHOD_CONFLICT`）：材料分别出现线性插值与邻近5点IDW；本项目的正式规则是不执行这两种论文清洗，除`1.#QNAN0`外保留全部有限值。
...
1. 更新配置schema，明确局部坐标约定、深度公式、Vx单位和无效值规则；
2. 生成可追溯的 `X/Y/depth_m/z_local_m/Vx` 标准表；
3. 增加坐标、距离、符号、2,005条建模记录和原文件哈希回归测试；
